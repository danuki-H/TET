...
window.addEventListener('scroll', function(){
    ...
    let rect = bgThiep.getBoundingClientRect();
    ...
    lan.style.transform = 'translateY(' + (-position*0.7) + 'px)';
    if(position >= rect.height/2){
        title.style.left = '50%'
    }
    if(position >= rect.height*1.2){
        title.style.left = '-100%'
    }
    if(position < rect.height/2){
        title.style.left = '140%'
    }
    ch1.style.transform = 'translateY(' + position*0.6 + 'px)'
    ch2.style.transform = 'translateY(' + -position*0.4 + 'px)'
